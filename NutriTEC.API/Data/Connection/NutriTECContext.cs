...
namespace NutriTEC.API.Data
{
    public class NutriTECContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Usuario>()
                .Property(u => u.Fecha_nacimiento).HasColumnType("date");
            ...
            modelBuilder.Entity<Admin>()
                .HasKey(a => a.Id_admin);
            modelBuilder.Entity<Admin>()
                .HasIndex(a => a.Correo).IsUnique();

            // ------------------------------------------------------------
            // Nutricionista
            // ------------------------------------------------------------
            modelBuilder.Entity<Nutricionista>()
                .HasKey(n => n.Id_usuario);
            modelBuilder.Entity<Nutricionista>()
                .Ignore(n => n.Correo)
                .Ignore(n => n.Contrasena)
                .Ignore(n => n.Nombre)
                .Ignore(n => n.Ap1)
                .Ignore(n => n.Ap2)
                .Ignore(n => n.Fecha_nacimiento)
                .Ignore(n => n.Peso)
                .Ignore(n => n.Altura);
            ...
            modelBuilder.Entity<Cliente>()
                .Ignore(c => c.Correo)
                .Ignore(c => c.Contrasena)
                .Ignore(c => c.Nombre)
                .Ignore(c => c.Ap1)
                .Ignore(c => c.Ap2)
                .Ignore(c => c.Fecha_nacimiento)
                .Ignore(c => c.Peso)
                .Ignore(c => c.Altura);

            // ------------------------------------------------------------
            // ClientexNutricionista — PK compuesta
            // ------------------------------------------------------------
            ...
            modelBuilder.Entity<Medida>()
                .HasKey(m => m.Id_medida);
            modelBuilder.Entity<Medida>()
                .Property(m => m.Fecha).HasColumnType("date");
            ...
            modelBuilder.Entity<Producto>()
                .Property(p => p.Estado).HasDefaultValue("pendiente");
            modelBuilder.Entity<Producto>()
                .Ignore(p => p.Vitaminas);

            // ------------------------------------------------------------
            // VitaminaxProducto — PK compuesta
            // ------------------------------------------------------------
            modelBuilder.Entity<VitaminaxProducto>()
                .HasKey(v => new { v.Id_producto, v.Vitamina });
            modelBuilder.Entity<VitaminaxProducto>()
                .ToTable("VitaminasxProducto");

            // ------------------------------------------------------------
            // Receta
            // ------------------------------------------------------------
            modelBuilder.Entity<Receta>()
                .HasKey(r => r.Id_receta);
            modelBuilder.Entity<Receta>()
                .Ignore(r => r.Productos);

            // ------------------------------------------------------------
            // ProductoxReceta — PK compuesta
            // ------------------------------------------------------------
            modelBuilder.Entity<ProductoxReceta>()
                .HasKey(pr => new { pr.Id_receta, pr.Id_producto });
            modelBuilder.Entity<ProductoxReceta>()
                .Ignore(pr => pr.Descripcion)
                .Ignore(pr => pr.Porcion)
                .Ignore(pr => pr.Energia)
                .Ignore(pr => pr.Grasa)
                .Ignore(pr => pr.Sodio)
                .Ignore(pr => pr.Carbohidratos)
                .Ignore(pr => pr.Proteina)
                .Ignore(pr => pr.Calcio)
                .Ignore(pr => pr.Hierro)
            ...
            modelBuilder.Entity<RegistroDiario>()
                .ToTable("Registro_Diario");
            modelBuilder.Entity<RegistroDiario>()
                .Property(r => r.Fecha).HasColumnType("date");
            modelBuilder.Entity<RegistroDiario>()
                .Ignore(r => r.Productos);

            // ------------------------------------------------------------
            // RegistroxProducto — PK compuesta
            // ------------------------------------------------------------
