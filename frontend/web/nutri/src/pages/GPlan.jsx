...
// Reemplazar por el id del nutricionista autenticado cuando exista login/sesión.
...
const GestionPlan = () => {
  ...
  const cargarPlanesExistentes = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/plan/nutricionista/${ID_NUTRICIONISTA_ACTUAL}`);
  ...
  const handleVerDetallePlan = async (idPlan) => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/plan/${idPlan}`);
  ...
  const handleCrearPlan = async () => {
    ...
    try {
      ...
      const cuerpoPayload = {
        ...
        id_nutricionista: ID_NUTRICIONISTA_ACTUAL,
      ...
      const url = esEdicion ? `${API_BASE_URL}/plan/${idPlanEditando}` : `${API_BASE_URL}/plan`;
  ...
  const handleEliminarPlan = async (idPlan, nombre) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/plan/${idPlan}`, { method: "DELETE" });
