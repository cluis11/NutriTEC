...
const ReporteMedidas = () => {
    ...
    // Carga el historial completo de medidas
    const cargarHistorial = async () => {
        try {
            const usuarioLogueado = JSON.parse(localStorage.getItem("usuario")) || {};
            const idActual = usuarioLogueado.id || idUsuario;
            setIdUsuario(idActual);

            const response = await fetch(`http://localhost:5108/api/medida/usuario/${idActual}`);
            if (response.ok) {
    ...
    // Transformar fecha ISO a formato dd/mm/yyyy 
    ...
    return (
        ...
        <style>
            ...
            @media print {
                ...
                #area-imprimible-reporte, #area-imprimible-reporte * {
                    visibility: visible;
                }
                #area-imprimible-reporte {
                    position: absolute;
                    left: 0;
                    top: 0;
                    width: 100%;
        ...
        <div className="card border-0 shadow-sm p-4 bg-white rounded-3 mb-4 d-print-none">
            ...
            <div className="col-12 col-sm-4">
                ...
                className="btn text-white w-100 py-2 fw-medium shadow-sm"
        ...
        <div id="area-imprimible-reporte" className="card border-0 shadow-sm p-4 bg-white rounded-3">
            ...
            <table className="table table-sm small align-middle mb-0">
                ...
                {medidasFiltradas.length === 0 ? (
                    <tr>
                    <td colSpan="6" className="text-muted text-center py-5 font-monospace">
                        No se registran medidas en el periodo seleccionado.
                    </td>
                    </tr>
                ) : (
                    medidasFiltradas.map((m, index) => (
                    <tr key={index} className="border-bottom" style={{ borderColor: "#f1f5f9" }}>
                        <td className="py-2.5 fw-medium font-monospace" style={{ color: "#0f172a" }}>
                        {formatearFecha(m.fecha)}
                        </td>
                        <td className="py-2.5 text-center font-monospace text-secondary">{m.cintura} cm</td>
                        <td className="py-2.5 text-center font-monospace text-secondary">{m.cuello} cm</td>
                        <td className="py-2.5 text-center font-monospace text-secondary">{m.caderas} cm</td>
                        <td className="py-2.5 text-center font-monospace fw-semibold text-success">{m.p_musculo}%</td>
                        <td className="py-2.5 text-center font-monospace fw-semibold text-danger">{m.p_grasa}%</td>
                    </tr>
                    ))
                )}
                </tbody>
